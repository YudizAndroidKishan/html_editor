        ...
        if (t.bold || t.italic || t.underline || t.clearAll) {
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              ...
              if (t.getIcons1()[index].icon == Icons.h_mobiledata) {
                var proceed = await widget.htmlToolbarOptions.onButtonPressed
                    ?.call(ButtonType.h1, _fontSelected[index],
                    updateStatus) ??
                      true;
                if (proceed) {
                  // Change this line to apply H1 instead of bold
                  widget.controller.execCommand('formatBlock', argument: 'h1');
                  updateStatus();
                }
              }
